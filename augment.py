        ...
        for file in filenames:
            ...
            if feats > params["min_features"]:
                if args["show"]:
                    cv2.imshow("debug", im)
                    cv2.waitKey(0)
                augmented_im = adjust_saturation(im, random.uniform(params["saturation_scale"],1,2))
                augmented_im = adjust_contrast(augmented_im, random.uniform(params["contrast_alpha"],1,2))
                augmented_im = gaussian_noise(augmented_im, params["gaussian_noise"])
                cv2.imwrite(f"{proc_dataset_path}/{label}/{cnt}.jpg", im)
                cnt += 1
                cv2.imwrite(f"{proc_dataset_path}/{label}/{cnt}.jpg", augmented_im)
                cnt += 1
